...
- 用 `/` 选中一条提示词后，不再把整段正文粘进输入框，只在开头留下 `/名字` 这一小截文本——字号与其它文字一致，不额外占一行也不撑高输入框，再长的模板都不会把你正要写的话淹掉。接着在它后面写自己的要求再发送，也可以只留着 `/名字` 直接发（此时提示词本身就是这一轮的请求）。它是一整块：光标落在上面按退格或 Delete 会一次性整段删掉，删掉即不再携带这条提示词。发出去的消息上带一枚 `/名字` 标签，点开可查看本次实际发出的提示词内容 ([#53](https://github.com/maotoumao/Cebian/issues/53))
...
- Picking a prompt with `/` no longer pastes the whole body into the input box. All that stays behind is a short `/name` at the start of your text — same type size as everything else, taking no extra line and not growing the input box, so a long template can't bury the message you're actually writing. Type your own request after it and send, or send with just the `/name` and the prompt becomes the request. It behaves as one unit: pressing Backspace or Delete on it removes the whole thing at once, and removing it drops the prompt. The sent message carries a `/name` tag — click it to see exactly what was sent ([#53](https://github.com/maotoumao/Cebian/issues/53))
