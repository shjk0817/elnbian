...
Cebian 不會把正文貼進輸入框，而是在輸入框首行左端掛一枚帶色的 `/名稱` 標記——再長的模板也不會把你正要寫的話淹沒。你可以在它右邊接著寫自己的要求再傳送；也可以只掛著標記直接傳送，此時 Prompt 本身就是這一輪的請求。

標記不是文字：你選不中它的一部分，游標也進不去。把游標移到文字最前面按一下倒退鍵，就能整個移除。移除之後，這則訊息就不再攜帶這個 Prompt。

傳送後，對話裡的這則訊息就是以 `/名稱` 開頭、後面接你自己打的話，和普通文字一個樣式。Prompt 正文不會在氣泡裡展開——它是 Cebian 實際送給模型的內容。
