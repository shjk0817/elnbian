...
Prompt 會以一枚 `/名稱` 的小標籤掛在輸入框上方，而不是把正文貼進輸入框——再長的模板也不會把你正要寫的話淹沒。你可以在旁邊接著寫自己的要求再傳送；也可以只掛著標籤直接傳送，此時 Prompt 本身就是這一輪的請求。

點標籤上的 × 可以移除它。**按兩下標籤**則把 Prompt 正文展開回輸入框，方便你傳送前先改。

傳送後，對話裡的這則訊息同樣帶著 `/名稱` 標籤，點開就能看到這次實際送出的 Prompt 內容。
...
<QA>
	...
	<QAItem q="我想像以前那樣在傳送前改 Prompt。">按兩下那枚標籤，正文就會展開回輸入框，變成普通文字任你編輯。</QAItem>
