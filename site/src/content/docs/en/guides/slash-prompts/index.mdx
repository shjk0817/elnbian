...
The Prompt is attached as a small `/name` chip above the input box rather than pasted in as text, so a long template doesn't bury the message you're actually writing. Type your own request next to it and send — or send with just the chip, and the Prompt itself becomes the request.

Click the × on the chip to detach it. **Double-click the chip** to expand the Prompt back into the input box as plain text, if you'd rather edit it before sending.

In the conversation, the message shows the same `/name` chip; click it to see the exact Prompt content that was sent.
...
<QAItem q="I want to edit the Prompt before sending, like before.">Double-click the chip and its content expands back into the input box as ordinary text.</QAItem>
