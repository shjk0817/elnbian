...
Rather than pasting the body in, Cebian leaves a short `/name` at the start of your text, so a long template doesn't bury the message you're actually writing. Type your own request after it and send — or send with just the `/name`, and the Prompt itself becomes the request.

It behaves as one unit: put the caret on it and press Backspace (or Delete) to remove the whole thing at once. Once it's gone, the message no longer carries the Prompt.

In the conversation, the sent message shows a `/name` tag; click it to see the exact Prompt content that was sent.
